...
export class TextColor {
    ...
    tileStyle(selectedComponent, colorValue) {
        var _a;
        const iconPath = selectedComponent.find('.tile-icon')[0];
        if (iconPath) {
            const svgElement = iconPath && iconPath.view.el.querySelector('svg');
            if (svgElement) {
                (_a = svgElement.querySelector('path')) === null || _a === void 0 ? void 0 : _a.setAttribute('fill', colorValue);
            }
        }
        ...
        if (pageData.PageType === "Information") {
            ...
            infoSectionController.updateInfoTileAttributes(selectedComponent.parent().parent().getId(), selectedComponent.parent().getId(), "Color", colorValue);
        }
        else {
            globalThis.tileMapper.updateTile(selectedComponent.parent().getId(), "Color", colorValue);
        }
    }
    ctaStyle(selectedComponent, colorValue) {
        ...
        if (pageData.PageType === "Information") {
            ...
            new ContentMapper(pageId).updateContentCtaColor(selectedComponent.getId(), colorValue);
        }
    }
    render(container) {
