...
export class PageTree {
    ...
    build() {
        ...
        this.graphContainer = document.getElementById("graph-container-1");
    ...
    buildTree() {
        ...
        this.node.append("rect")
            ...
            .attr("y", -20)
            .style("stroke", (d) => this.getNodeColor(d.status))
        ...
        this.simulation = this.d3.forceSimulation(this.graphData.nodes)
            .force("link", this.d3.forceLink(this.graphData.links).id((d) => d.id).distance(150))
        ...
        this.d3.select("#reset").on("click", () => {
            svg.transition().duration(700).call(zoom.transform, this.d3.zoomIdentity.translate(width / 2, height / 2).scale(0.8));
        });
        // Toggle layout
        ...
        this.d3.select("#add-node").on("click", () => {
            ...
            const parentNode = this.graphData.nodes[parentIndex];
            const newId = (Math.max(...this.graphData.nodes.map((n) => parseInt(n.id))) + 1).toString();
            const newNode = {
                id: newId,
        ...
        svg.call(zoom.transform, this.d3.zoomIdentity.translate(width / 2, height / 2).scale(0.8));
        this.toggleLayout();
    }
    show() {
    ...
    ticked() {
        this.link.attr("d", (d) => {
            const sourceNode = this.graphData.nodes.find((node) => node.id === d.source.id || node.id === d.source);
            const targetNode = this.graphData.nodes.find((node) => node.id === d.target.id || node.id === d.target);
    ...
    showPopover(event, d) {
        ...
        if (d.parentId) {
            const parentNode = this.graphData.nodes.find((node) => node.id === d.parentId);
            if (parentNode)
    ...
    assignCoordinates(nodes) {
        ...
        nodes.forEach((node) => nodeMap.set(node.id, Object.assign({}, node)));
        const roots = nodes.filter((node) => !nodes.some((n) => n.children.includes(node.id)));
        let yCounter = -500;
        function setPosition(nodeId, depth) {
