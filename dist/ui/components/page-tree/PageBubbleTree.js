...
export class PageBubbleTree {
    constructor() {
        ...
        console.log('processed pages', this.processedPages.map((page) => page.title).sort());
                ...
                page.PageMenuStructure.Rows.forEach((row) => {
                    row.Tiles.forEach((tile) => {
                        if (tile.Action.ObjectType == "DynamicForm" || tile.Action.ObjectType == "WebLink") {
                            ...
                            if (this.pages.filter((page) => page.PageId === tile.Action.ObjectId)) {
                                ret.children.push(tile.Action.ObjectId);
                            }
                        }
                    });
    ...
    createNodes(processedPages = this.processedPages) {
        return processedPages.map(p => ({ id: p.id, name: p.title, children: p.children }));
    }
    createLinks(processedPages = this.processedPages) {
        return processedPages.flatMap(p => p.children.map((childId) => ({
            source: p.id,
            target: childId
        })));
    }
    ...
    forceSimulation() {
        this.simulation = this.d3.forceSimulation(this.nodes)
            .force("link", this.d3.forceLink(this.normalLinks).id((d) => d.id).distance(400))
    ...
    onNodeClick(event, d) {
        let nodesToAdd = [d.id];
        const processedPages = this.processedPages.filter((page) => page.id === d.id);
        const childIds = d.children;
        while (childIds.length > 0) {
            // for each childId, add to nodesToAdd
            const childId = childIds.pop();
            if (nodesToAdd.includes(childId))
                continue;
            nodesToAdd.push(childId);
            const childPage = this.processedPages.find((page) => page.id === childId);
            console.log(childPage === null || childPage === void 0 ? void 0 : childPage.title);
            if (childPage && childPage.children) {
                processedPages.push(childPage);
                childIds.push(...childPage.children);
            }
        }
        console.log('processed pages', processedPages.map((page) => page.title).sort());
...
//# sourceMappingURL=PageBubbleTree.js.map